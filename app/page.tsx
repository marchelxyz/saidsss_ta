...
export default async function Home() {
  const pool = getPool();
  const [articlesResult, casesResult, settingsResult, homePageResult] = await Promise.all([
    pool.query(
  ...
  const homePageId = homePageResult.rows[0]?.id as string | undefined;
  const homeBlocksResult = homePageId
    ? await pool.query(
    ...
    : { rows: [] };
  const useBuilder = homeBlocksResult.rows.length > 0;
